...
def test_PLACE_SHIPS_looks_reasonable_TOP_BOTTOM(n=1000):
    w = 10
    h = 10
    cumulative_ships = np.zeros((h, w), dtype=np.int32)
    env = BattleshipEnv(width=w, height=h)
    ...
    for _ in range(n):
        env.reset(favor_top=100, favor_bottom=100,
                  gradient_coef=lambda x: x**20)
        cumulative_ships += env.state > 0
...
def test_PLACE_SHIPS_looks_reasonable_CENTER(n=1000):
    ...
    for _ in range(n):
        env.reset(favor_top=-10000, favor_right=-10000,
                  favor_left=-10000, favor_bottom=-10000)
        cumulative_ships += env.state > 0
            ...
            for element in row:
                if element != 0:
                    if element in seen_in_previous_rows:
                        ...
                        raise Exception(
                            "BAD!!! Saw a %s in multiple rows." % element)
                    else:
...
# test_PLACE_SHIPS_looks_reasonable_TOP()
# test_PLACE_SHIPS_looks_reasonable_TOP_BOTTOM()
