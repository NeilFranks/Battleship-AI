...
class BattleShipEnv(Env):
    ...
    def _get_init_state(self):
        bsstate = ShipState()
        self.grid.build_board()
        valid_pos = [i for i in range(self.grid.n_tiles)]  # valid poses to sample from
        tested_invalid_poses = []
        for length in reversed(range(1, self.max_len)):
            
            tested_invalid_poses.clear()
            legal_ship_found = False
            
            while not legal_ship_found:  # add one ship of each kind
                pos = valid_pos.pop(np.random.randint(len(valid_pos)))   # sample a new pos 
                valid_direction = [j for j in range(4)] # valid directions to sample from
                ship = Ship(coord=self.grid.get_coord(pos), length=length)
                direction_found = False
                while (not direction_found) and (not len(valid_direction) == 0):
                    dirc = valid_direction.pop(np.random.randint(len(valid_direction))) # sample a direction

                    ship.direction = dirc
                    if not self.collision(ship, self.grid, bsstate):
                        legal_ship_found = True
                        direction_found = True
                if legal_ship_found == False:
                    tested_invalid_poses.append(pos)
            
            valid_pos.extend(tested_invalid_poses)  #return the tested invalid poses to the valid poses
            #remove ship's occupied cells from valid_pos
            ship_coord = ship.pos 

            for i in range(ship.length): 
                ship_coord += Compass.get_coord(ship.direction)
                valid_pos.remove(self.grid.get_index(ship_coord))
...
class po_rollout(object):
    def __init__(self, env, rollout_mode = "random", num_particle = 1000, num_sim = 1000):
        ...
        self.mode = rollout_mode           # rollout action search modes: 1. random, 2. hunt-n-target
        self.no_ship_poses = []            # locations where a ship cell does not exist
        self.yes_ship_poses = []           # locations where a ship cell does exist
        self.current_grid = []             # observed grid
        

    def _init_solver(self,board_size):
        self.current_grid = BattleGrid(board_size)
        scenarios = []
    ...
    def _rollout(self, action_set):       
        if self.mode == "random":
            tot_rw = self._random_rollout(action_set)
        elif self.mode == "hunt-n-target":
            tot_rw = self._hunt_n_target_rollout(action_set)
        else:
            raise Exception('Selected rollout mode ({}) is not correct'.format(self.mode))
        return tot_rw
    
    def _hunt_n_target_rollout(self, action_set):
        done = False
        tot_rw = 0
        action_stack = []
        
        # apply the first action in action set
        action = action_set.pop(0)
        ob , rw , done = self._generative_model(action)
        tot_rw += rw
        if done:
            return tot_rw

        self._explore_target(ob, action, action_stack, action_set)

        while not done:
            if len(action_stack) == 0: # hunt mode
                action = action_set.pop(0)
                ob , rw , done = self._generative_model(action)
                tot_rw += rw
                self._explore_target(ob, action, action_stack, action_set)
    ...
    def _explore_target(self, obs, action, action_stack, action_set):
        if obs == 1: #target
            coord = self.sim_env.grid.get_coord(action)
            for adj in range(4):
                temp_coord = coord + Compass.get_coord(adj) 
                if self.sim_env.grid.is_inside(temp_coord):
                    cell = self.sim_env.grid[temp_coord]
                    if not cell.visited:
                        temp_action = self.sim_env.grid.get_index(temp_coord)
                        if temp_action in action_set:
                            action_set.remove(temp_action)
                            action_stack.append(temp_action)


    def _random_rollout(self, action_set):
        done = False
        tot_rw = 0
    ...
    def _belief_update(self, action, ob):
        self._update_current_grid(action, ob)  
        updated_belief = []
        ...
        if K==self.num_particle:  # particles full
            ...
            while K != self.num_particle:
                state = self._particle_reinvig(updated_belief)
                updated_belief.append(state)
                K += 1
            self.belief = updated_belief
            return no_belief
    
    def _particle_reinvig(self, updated_belief):
        temp = np.random.randint(0,3)
        if temp <=2: #selection prob = 3/3
            state = self._random_matching_reinvig_favor_occupy()  
        else:        #selection prob = 0/3
            state = self._random_matching_reinvig()
        return state

    def _random_matching_reinvig_favor_occupy(self):
        if len(self.yes_ship_poses) == 0:
            return self._random_matching_reinvig()

        obs_grid = self.current_grid
        valid_pos = [i for i in range(obs_grid.n_tiles)]
        valid_favored_pos = []
        tested_invalid_poses = []
        tested_invalid_favored_poses = []
        for pos in self.yes_ship_poses:
            valid_favored_pos.append(pos)
        for pos in self.no_ship_poses:
            valid_pos.remove(pos)
        
        bsstate = ShipState()
        sim_env = self.sim_env
        sim_env.grid.build_board()

        for length in reversed(range(1, sim_env.max_len)):
            
            tested_invalid_poses.clear()
            tested_invalid_favored_poses.clear()
            legal_ship_found = False

            while not legal_ship_found:  # add one ship of each kind
                if len(valid_favored_pos) == 0:
                    pos = valid_pos.pop(np.random.randint(len(valid_pos)))   # sample a new pos
                else: 
                    pos = valid_favored_pos.pop(np.random.randint(len(valid_favored_pos)))   # sample a new valid pos
                    valid_pos.remove(pos)

                valid_direction = [j for j in range(4)] # valid directions to sample from
                ship = Ship(coord = sim_env.grid.get_coord(pos), length=length)
                direction_found = False
                while (not direction_found) and (not len(valid_direction) == 0):
                    dirc = valid_direction.pop(np.random.randint(len(valid_direction))) # sample a direction

                    ship.direction = dirc
                    if not sim_env.collision(ship, sim_env.grid, bsstate):
                        ship_compatiable_with_observ = True
                        ship_coord = ship.pos 
                        for i in range(ship.length): 
                            ship_coord += Compass.get_coord(ship.direction)
                            if (obs_grid[ship_coord].visited) and (not obs_grid[ship_coord].occupied):
                                ship_compatiable_with_observ = False

                        if ship_compatiable_with_observ:
                            legal_ship_found = True
                            direction_found = True
                if legal_ship_found == False:
                    tested_invalid_poses.append(pos)
                    if pos in self.yes_ship_poses:
                        tested_invalid_favored_poses.append(pos)
            
            valid_pos.extend(tested_invalid_poses)  # return the tested invalid poses to the valid poses
            valid_favored_pos.extend(tested_invalid_favored_poses)  # return the tested invalid poses to the valid poses
            # remove ship's occupied cells from valid_pos
            ship_coord = ship.pos 

            for i in range(ship.length): 
                ship_coord += Compass.get_coord(ship.direction)
                ship_idx = sim_env.grid.get_index(ship_coord)
                valid_pos.remove(ship_idx)
                if ship_idx in valid_favored_pos:
                    valid_favored_pos.remove(ship_idx)

            sim_env.mark_ship(ship, sim_env.grid, bsstate)
            bsstate.ships.append(ship)
        return bsstate

    def _random_matching_reinvig(self):
        obs_grid = self.current_grid
        valid_pos = [i for i in range(obs_grid.n_tiles)]
        tested_invalid_poses = []
        for pos in self.no_ship_poses:
        ...
        for length in reversed(range(1, sim_env.max_len)):
            ...
            for i in range(ship.length): 
                ship_coord += Compass.get_coord(ship.direction)
                valid_pos.remove(sim_env.grid.get_index(ship_coord))
    ...
    def _update_current_grid(self, action, ob):
        coord = self.current_grid.get_coord(action)
        self.current_grid[coord].visited = True
        if ob == 1:
            self.current_grid[coord].occupied = 1
            self.yes_ship_poses.append(action)
        else:
            self.no_ship_poses.append(action)

def play(board_size, max_ship_len, rollout_mode, num_particle, num_sim):
    ...
    solver = po_rollout(copy.deepcopy(env), rollout_mode = rollout_mode, num_particle = num_particle, num_sim = num_sim) 
    solver._init_solver(board_size)   # initialize the POMDP solver (initialize belief set)
    ...
    while not done:
        ...
        t += 1
        if t%10==0:
            print("iteration:{}, remaining ship cells: {}".format(t,env.state.total_remaining))
    env.close()
...
if __name__ == "__main__":
    ...
    board_size = (10, 10)  # grid_size 5*5
    max_ship_len = 5    # ship lengths 1*2 and  1*3 

    rollout_mode = "hunt-n-target" 
    num_particle = 1000
    num_sim = [10000]
    avg_rw = np.zeros((len(num_sim)))
    num_games = 20
    for s in range(len(num_sim)):
        for j in range(num_games):
            print("num_sim: {}, game: {}/{}".format(num_sim[s], j+1, num_games))
            tot_rw = play(board_size, max_ship_len, rollout_mode, num_particle, num_sim[s])
            avg_rw[s] += tot_rw
        avg_rw[s] = avg_rw[s]/num_games
        print("num_sim = {}, average rewards: {}".format(num_sim[s], avg_rw[s]))
    
    num_sim = np.array(num_sim)
    plt.plot(num_sim, avg_rw, marker = "s")
    plt.xlabel('num_sim')
